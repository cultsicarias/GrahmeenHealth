...
export default function DoctorDashboardContent() {
  ...
  const handleSendSMS = async () => {
    try {
      const response = await fetch('/api/send-doctor-sms', {
        method: 'POST',
      });
      ...
      <div className="space-y-4">
        {loading ? (
          ...
          <div className="space-y-8">
            {Object.entries(groupReportsByDate(reports)).map(([date, dateReports]) => (
              <div key={date} className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {dateReports.map((report) => (
                    <div
                      key={report.id}
                      className="bg-white shadow-sm border border-gray-100 rounded-lg p-6 hover:shadow-md transition-shadow relative"
                    >
                      <button
